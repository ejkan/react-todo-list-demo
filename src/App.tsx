...
type FilterType = 'all' | 'active' | 'completed'; // FilterType is an enumerate type.
...
function App() {
  ...
  const [inputValue, setInputValue] = useState<string>(() => {
    const savedInputValue = localStorage.getItem('inputValue'); // Get saved input value from localStorage.
    return savedInputValue ? JSON.parse(savedInputValue) : ''; // Parse savedInput value to JSON.
  })
    ...
    <div className="todo-container">
      ...
      <div className="filter-tabs">
        ...
        <button
          ...
          {/* Means 'assigned' ? boolean and 'events'. */}
      ...
      <ul className="todo-list">
        {filteredTodos.length === 0 ? (
          ...
          <li className="empty-state">
            ...
            <p>
              {filter === 'all' && 'ยังไม่มีรายการ ลองเพิ่มสักรายการมั้ย 💭'}
              {filter === 'active' && 'ไม่มีรายการที่ต้องทำเเล้ว 🫶🏼'}
              {filter === 'completed' && 'ยังไม่มีรายการที่เสร็จสักอย่างเลย 😥'}
            </p>
          </li>
        ) : (
      ...
      {completedCount > 0 && (
        <div className="todo-footer">
          <button
            ...
            aria-label='ล้างรายการที่เสร็จแล้ว'
          >
            <span className="btn-text">🗑️ ลบรายการที่เสร็จแล้ว {completedCount} รายการ</span>
          </button>
        </div>
      )}
    </div>
